...
def setup_setting_files(seed, max_steps):
    ...
    # PDA Lunar_Lander with linear
    od['alg'] = 'pmd'
    od['env_name'] = 'LunarLander-v2'
    od['pmd_stepsize_type'] = 'pda_1' 
    od['pmd_fa_type'] = "linear"
    od['pmd_pe_alpha'] = 1e-4
    ...
    # PDA Lunar_lander with linear with SB3
    od['pmd_sb3_policy'] = True
    fname = os.path.join(folder_name, "run_%s.json" % ct)
    if not(os.path.exists(od["log_folder"])):
        os.makedirs(od["log_folder"])
    with open(fname, 'w', encoding='utf-8') as f:
    ...
    # PDA Lunar_lander with nn with SB3
    od['pmd_sb3_policy'] = True
    fname = os.path.join(folder_name, "run_%s.json" % ct)
    if not(os.path.exists(od["log_folder"])):
        os.makedirs(od["log_folder"])
    ...
    # PDA Lunar_Lander perturbed with linear
    od['lunar_perturbed'] = True
    od['pmd_fa_type'] = "linear"
    od['pmd_pe_alpha'] = 1e-4
    od['pmd_pe_stepsize_base'] = 0.01
    fname = os.path.join(folder_name, "run_%s.json" % ct)
    ...
    # PDA Lunar_lander perturbed with nn 
    od['pmd_fa_type'] = "nn"
    od['pmd_pe_alpha'] = 0.0
    od['pmd_pe_stepsize_base'] = 0.001
    fname = os.path.join(folder_name, "run_%s.json" % ct)
