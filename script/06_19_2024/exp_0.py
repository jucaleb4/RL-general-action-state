...
def setup_setting_files(seed_0, max_trials, max_steps):
    ...
    pda_policy_noises = [0.01, 0.1]

    for pda_policy_noise in pda_policy_noises:
        od['pda_policy_noise'] = pda_policy_noise
    ...
    assert ct == MAX_RUNS, "#experiments (%i) does not matches expected #runs (%i)" % (ct, MAX_RUNS)
